    ...
    if(len(context.args) == 2):
        ...
        if(isinstance(functionsDict, str)):
            ...
            for functionDate in functionsDict:
                functionsMessage += "*bold****{:0}***\n".format(functionDate)
                
                for functionFormat, functionHours in functionsDict.items():
...
async def functions_date_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if(len(context.args) == 3):
        ...
        functionsDict = cinemark.get_today_functions(context.args[0], context.args[1], context.args[2])
        if(isinstance(functionsDict, str)):
            functionsMessage = functionsDict
        else:
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("getcinemasformovie", cinemas_movie_command , pass_args=True))
    app.add_handler(CommandHandler("getmoviesforcinema", movies_cinema_command, pass_args=True))
    app.add_handler(CommandHandler("getdates", dates_movie_command, pass_args=True))
    app.add_handler(CommandHandler("gettodayfunctions", today_functions_command, pass_args=True))
    app.add_handler(CommandHandler("getfunctionsfordate", functions_date_command, pass_args=True))
